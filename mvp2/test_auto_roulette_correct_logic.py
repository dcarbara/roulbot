...
def test_auto_roulette_correct_logic():
    """Test the correct auto roulette logic"""
    print("🧪 Testing Correct Auto Roulette Logic")
    print("=" * 50)
    ...
    # Test sequence: [1, 3] -> WIN -> [22, 15] -> WIN -> [7, 9] -> WIN -> [11]
    test_sequence = [
        (1, False),   # Loss
        (3, False),   # Loss  
        (15, True),   # WIN! (15 is in street 1, same as 3) - First win
        (22, False),  # Loss - Continue placing bets
        (13, False),  # Loss - Continue placing bets
        (33, True),   # WIN! (33 is in street 3, same as 13) - Second win, should wait
        (7, False),   # Loss - Should wait for new pattern
        (9, False),   # Loss - Should wait for new pattern
        (11, True),   # WIN! (11 is in street 4, same as 9) - First win again
    ]
    
    print(f"\n📊 Testing sequence: {[f'{n}({"W" if w else "L"})' for n, w in test_sequence]}")
    print("Format: Number -> Street -> Bet Labels -> Win/Loss -> Action")
    ...
    for i, (number, is_win) in enumerate(test_sequence):
        ...
        # Check if we need to wait for new pattern
        if is_win:
...
if __name__ == "__main__":
    test_auto_roulette_correct_logic() 