...
class RouletteBacktester:
    ...
    def backtest_strategy(self, 
        ...
        while True:
            ...
            # Multi-label strategies: check the TOTAL across all labels, not just
            # one label's amount. Without this, a 5-label Fibonacci at level 10
            # ($44.50/label = $222.50 total) passes the per-label check ($44.50 < bal)
            # but then bankrupts the session by $100+.
            if current_total_bet > current_balance:
                session_stop_reason = "INSUFFICIENT_BALANCE"
                session_stop_message = (f"Round {round_num}: total bet ${current_total_bet:.2f} "
                                        f"exceeds balance ${current_balance:.2f}")
                break
