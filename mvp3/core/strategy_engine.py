...
class StrategyEngine:
    ...
    def _load_progression(self, progression_type, base_bet, dynamic_rules, session_start_balance, custom_sequence, dalembert_step):
        if progression_type == "dynamic":
            ...
            return FibonacciStrategy(base_bet, session_start_balance=session_start_balance)
    ...
    def record_result(self, win: bool, current_balance: float = None, winning_number=None,
        ...
        if isinstance(self.progression, DynamicProgressionStrategy):
            ...
            _sig = _inspect.signature(self.progression.record_result)
            if 'current_profit' in _sig.parameters and current_profit is not None:
                _kwargs = {'current_profit': current_profit}
                if 'round_pnl' in _sig.parameters and round_pnl is not None:
                    _kwargs['round_pnl'] = round_pnl
                self.progression.record_result(win, **_kwargs)
            else:
